...
export function RedPacket(props: RedPacketProps) {
    ...
    const shareLink = activatedSocialNetworkUI.utils
        .getShareLinkURL?.(
            canClaim
                ? [
                      `I just claimed a red packet from @${payload.sender.name}. Follow @subdao_network (subdao.network) to claim red packets.`,
                      '#subdao #RedPacket',
                      postLink,
                  ]
                      .filter(Boolean)
    ...
    useEffect(() => {
        ...
        canRefund,
        payload.sender.name,
        setTransactionDialogOpen /* update tx dialog only if state changed */,
    ])
    //#endregion
