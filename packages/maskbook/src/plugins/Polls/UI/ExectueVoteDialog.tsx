...
interface ExctueVoteDialogProps extends withClasses<'wrapper'> {
    poll: PollGunDB
    voteData: VoteData
    open: boolean
...
export default function ExctueVoteDialog(props: ExctueVoteDialogProps) {
    const classes = useStyles()
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true)
        const res = await Services.Polkadot.executeVote(vote_address, voteId)
        if (res) {
            setLoading(false)
            props.onDecline()
        }
    ...
    return (
        <InjectedDialog open={props.open} onClose={props.onDecline} title={t('plugin_poll_excute_dialog')}>
            <DialogContent>
                <CardUI>
                    <Typography variant="h4" className={classes.line}>
                        {t('plugin_poll_excuteing_confirm')}
                    </Typography>
                    <Typography className={classes.line}>{voteData?.title}</Typography>
                    <Accordion>
                        <AccordionSummary
