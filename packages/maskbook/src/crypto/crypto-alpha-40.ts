...
export async function encrypt1To1(info: {
    ...
    return { salt, encryptedContent, version: VERSION.validate }
}
...
export async function encrypt1ToN(info: {
    ...
    const othersAESKeyEncrypted = await generateOthersAESKeyEncrypted(
        VERSION.validate,
        AESKey,
        privateKeyECDH,
        othersPublicKeyECDH,
...
export async function decryptMessage1ToNByMyself(info: {
    ...
    const decryptedAESKey = await extractAESKeyInMessage(VERSION.validate, encryptedAESKey, iv, myLocalKey)
