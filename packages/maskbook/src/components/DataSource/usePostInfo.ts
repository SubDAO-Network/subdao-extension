...
import type { ProfileIdentifier } from '@subdao/maskbook-shared'
import { isValidVersion, isSharedPublicVersion } from '../../crypto/constants'
...
export function usePostInfoSharedPublic(): boolean {
    ...
    if (!isSharedPublicVersion(payload.version)) return false
...
export function usePostClaimedAuthor(): ProfileIdentifier | undefined {
    ...
    const payload = info.val
    if (!isValidVersion(payload.version)) return undefined
