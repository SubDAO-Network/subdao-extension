...
function parseText(string: string, allowTextEnlarge: boolean) {
    ...
    while (current.length) {
        ...
        const key = `${search1}-${search2}-${index}`
        index++
        // ? if rest is normal
        if (search1 === -1 && search2 === -1) {
            result.push(<ParseText key={key} text={current} fontSize={fontSize} />)
            break
        }
        // ? if rest have \n but no links
        if ((search1 < search2 && search1 !== -1) || search2 === -1) {
            result.push(
                <ParseText key={key} text={current.substring(0, search1)} fontSize={fontSize} />,
                <br key={current} />,
            )
        ...
        if ((search2 < search1 && search2 !== -1) || search1 === -1) {
            ...
            result.push(
                <ParseText key={key} text={current.substring(0, search2)} fontSize={fontSize} />,
