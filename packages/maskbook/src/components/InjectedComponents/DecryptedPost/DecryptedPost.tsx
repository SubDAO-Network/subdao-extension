...
export function DecryptPost(props: DecryptPostProps) {
    ...
    const authorInPayload = deconstructedPayload
        .andThen((x) => (isValidVersion(x.version) ? Ok(x.authorUserID) : Err.EMPTY))
        .unwrapOr(undefined)
    ...
    const decryptedPayloadForImageAlpha38 = isValidVersion(decryptedPayloadForImage?.version ?? 0)
        ? decryptedPayloadForImage
        : null
    const sharedPublic =
        deconstructedPayload
            .andThen((x) => (isValidVersion(x.version) ? Ok(!!x.sharedPublic) : Err.EMPTY))
            .unwrapOr(false) || decryptedPayloadForImageAlpha38?.sharedPublic
