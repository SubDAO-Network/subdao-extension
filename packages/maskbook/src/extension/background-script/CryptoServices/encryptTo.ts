...
import { constructAlpha20, PayloadLatest } from '../../../utils/type-transform/Payload'
...
const OthersAESKeyEncryptedMap = new Map<
    ...
    [string, Alpha20.PublishedAESKeyRecordV39OrV38[]]
>()
...
export async function encryptTo(
    ...
    const stringifiedContent = Alpha20.typedMessageStringify(content)
    const localKey = publicShared ? Alpha20.publicSharedAESKey : (await queryLocalKey(whoAmI))!
    const {
    ...
    return [constructAlpha20(payload, await encodeTextPayloadWorker(whoAmI)), postAESKeyToken]
}
...
export async function publishPostAESKey(iv: string) {
    ...
    return Gun2.publishPostAESKeyOnGun2(-20, iv, ...OthersAESKeyEncryptedMap.get(iv)!)
}
