...
import { getCryptoProvider, cryptoProviderTable } from './cryptoProviderTable'
...
import { isValidVersion, flagChart } from '../../../crypto/constants'
...
async function* decryptFromPayloadWithProgress_raw(
    ...
    const data = post
    const { version } = data
    const sharePublic = publicShared ?? data.sharedPublic ?? false
    ...
    if (isValidVersion(version)) {
        ...
        const ownersAESKeyEncrypted = data.AESKeyEncrypted ?? data.ownersAESKeyEncrypted
        ...
        if (isValidVersion(data.version) && data.authorPublicKey) {
        ...
        if (author.equals(whoAmI)) {
            ...
            // If remove this if block, SubDAO will search the key
        ...
        try {
            ...
            if (e.message === i18n.t('service_not_share_target')) {
                ...
                // What to do next: You can ask your friend to visit your profile page, so that their SubDAO extension will detect and add you to recipients.
...
async function* decryptFromImageUrlWithProgress_raw(
    ...
    if (post.indexOf(flagChart) !== 0 && !/https:\/\/.+\..+\/(\?post_v\d=)?%50(.+)%90/.test(post))
